...
function local_course_extender_extend_settings_navigation($settingsnav, $context)
{
    ...
    if ($settingnode = $settingsnav->find('courseadmin', navigation_node::TYPE_COURSE)) {
        ...
        $url = new moodle_url('/local/course_extender/index.php', ['id' => $PAGE->course->id]);
...
function course_extender_update($courseid, $keepcourse)
{
    ...
    $exists = $DB->get_record('local_course_extender', ['courseid' => $course->id]);
    if (!$exists) {
        $DB->insert_record('local_course_extender', ['courseid' => $course->id, 'term' => $currentterm, 'shortname' => $course->shortname, 'fullname' => $course->fullname, 'keepcourse' => $keepcourse]);

        return true;
    } else {
        $record = $DB->get_record('local_course_extender', ['courseid' => $course->id]);
        $DB->update_record('local_course_extender', ['id' => $record->id, 'keepcourse' => $keepcourse]);
